...
async fn creates_comparison_run_and_target_snapshots() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(targets[0].profile_snapshot_json.contains("\"provider\":\"openai\""));
    assert!(targets[0].profile_snapshot_json.contains("\"model_name\":\"gpt-5.4\""));
    assert!(targets[0].profile_snapshot_json.contains("\"base_url\":\"https://api.example.com/v1\""));
...
#[tokio::test]
async fn rejects_comparison_run_when_binding_does_not_exist() -> Result<(), Box<dyn std::error::Error>>
{
    let pool = support::create_test_pool().await?;
    let case_service = EvaluationCaseService::new(pool.clone());
